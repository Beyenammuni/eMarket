    ...
    : IIyzicoSubMerchantService
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        Console.WriteLine(
    $"IYZICO ApiKey: {_options.ApiKey[..Math.Min(12, _options.ApiKey.Length)]}...");

        Console.WriteLine(
            $"IYZICO SecretKey Length: {_options.SecretKey.Length}");

        Console.WriteLine(
            $"IYZICO BaseUrl: {_options.BaseUrl}");

        Console.WriteLine(
    $"IYZICO SecretKey StartsWith sandbox-: {_options.SecretKey.StartsWith("sandbox-", StringComparison.Ordinal)}");

        Console.WriteLine(
            $"IYZICO SecretKey HasLeadingOrTrailingWhitespace: {_options.SecretKey != _options.SecretKey.Trim()}");
...
var binRequest = new RetrieveBinNumberRequest
{
    Locale = Locale.TR.ToString(),
    ...
    BinNumber = "554960"
            ...
            var binResponse =
                await BinNumber.Retrieve(
                    binRequest,
                    options);

            Console.WriteLine(
                $"IYZICO BIN TEST: Status={binResponse.Status}, " +
                $"Code={binResponse.ErrorCode}, " +
                $"Message={binResponse.ErrorMessage}");
