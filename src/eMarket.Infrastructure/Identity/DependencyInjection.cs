        ...
        IConfiguration configuration)
    {
        ...
        services.AddMediatR(cfg =>
        {
            cfg.RegisterServicesFromAssembly(
                typeof(IAssemplyMarker).Assembly);
        });
        ...
        services.AddSingleton<ISubMerchantProvisioningQueue,
    SubMerchantProvisioningQueue>();

        services.AddScoped<IIyzicoSubMerchantService,
            IyzicoSubMerchantService>();

        services.AddHostedService<SubMerchantProvisioningWorker>();
        ...
        services.AddScoped<IAdminDashboardRepository,
            AdminDashboardRepository>();

        // Payment provider
        var paymentProvider =
            configuration["Payments:Provider"]?.Trim();
                ...
                StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException(
                "Payments:Provider must be configured as Iyzico.");
        }

        services.Configure<IyzicoOptions>(
            configuration.GetSection(
                IyzicoOptions.SectionName));

        var iyzicoApiKey =
            configuration["Payments:Iyzico:ApiKey"];

        var iyzicoSecretKey =
            configuration["Payments:Iyzico:SecretKey"];

        var iyzicoBaseUrl =
            configuration["Payments:Iyzico:BaseUrl"];

        if (string.IsNullOrWhiteSpace(iyzicoApiKey))
        {
            throw new InvalidOperationException(
                "Payments:Iyzico:ApiKey is not configured.");
        }

        if (string.IsNullOrWhiteSpace(iyzicoSecretKey))
        {
            throw new InvalidOperationException(
                "Payments:Iyzico:SecretKey is not configured.");
        }

        if (string.IsNullOrWhiteSpace(iyzicoBaseUrl))
        {
            throw new InvalidOperationException(
                "Payments:Iyzico:BaseUrl is not configured.");
        }
        ...
        services.AddScoped<ISubscriptionDbContext>(sp =>
            sp.GetRequiredService<AppDbContext>());

        services.AddHostedService<
            SubscriptionOrderBackgroundService>();
